...
for dirpath, dirnames, files in os.walk(input_data_dir):
    for filename in files:
        if filename.endswith(".fna"):
            ...
            strain = '_'.join(splitted_noext_name[2:])
            print(genus + ' ' + species + ' ' + strain)

            p1 = sub.run([
                'prokka',
                '--prefix', '_'.join(filter(None,[genus, species, strain])),
                '--fast',
                '--norna',
                '--notrna',
                '--outdir', os.path.join(output_data_dir, dirs[-1], '_'.join(filter(None, [genus, species, strain]))),
                '--genus', genus,
                '--species', species,
                '--strain', strain,
